...
HRESULT Engine::CFontMgr::Ready_Font(LPDIRECT3DDEVICE9 pGraphicDev, const _tchar* pFontTag, const _tchar* pFontType, const _uint& iWidth, const _uint& iHeight, const _uint& iWeight) {
	...
	if (pFont == nullptr)
	{
		pFont = CFont::Create(pGraphicDev, L"¹ÙÅÁ", iWidth, iHeight, iWeight);
...
void Engine::CFontMgr::Render_Font_Center(const _tchar* pFontTag, const _tchar* pString, const _vec2* pPos, D3DXCOLOR Color) {
	CFont*	pFont = Find_Font(pFontTag);
	NULL_CHECK(pFont);

	pFont->Render_Font_Center(pString, pPos, Color);
