...
CVolcano_Parts::CVolcano_Parts(LPDIRECT3DDEVICE9 pGraphicDev)
	: Engine::CGameObject(pGraphicDev)
{
	
}

CVolcano_Parts::~CVolcano_Parts(void)
{

}

HRESULT CVolcano_Parts::Ready_Object(void)
{
...
_int CVolcano_Parts::Update_Object(const _float& fTimeDelta)
{
...
void CVolcano_Parts::Render_Object(void)
{
...
void CVolcano_Parts::Set_HeightMap(const _tchar * _pPath)
{
	m_pBufferCom->Set_Height(_pPath);
	
}

void CVolcano_Parts::Set_Trans(_vec3 & _vPos)
{
	m_pTransform->Set_Trans(&_vPos);
}

vector<int> CVolcano_Parts::Get_HeightVector()
{
...
void CVolcano_Parts::Weld_Edge(CVolcano_Parts * _Dst, bool _bHorizontal)
{
...
HRESULT CVolcano_Parts::Add_Component(void)
{
	Engine::CComponent*		pComponent = nullptr;

	m_pBufferCom = CHeightCol::Create(m_pGraphicDev, 385, 385, 100);
	m_pBufferCom->Set_HeightRedBase(L"../../Asset/Terrain/Volcano_Land.bmp", 8000.f);
...
CVolcano_Parts* CVolcano_Parts::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CVolcano_Parts*		pInstance = new CVolcano_Parts(pGraphicDev);
