...
CBill_AlphaLine::CBill_AlphaLine(LPDIRECT3DDEVICE9 pGraphicDev)
	: Engine::CGameObject(pGraphicDev) {

}

CBill_AlphaLine::~CBill_AlphaLine(void) {

}

HRESULT CBill_AlphaLine::Ready_Object(void) {
...
int CBill_AlphaLine::Update_Object(const float& fTimeDelta) {
	...
	m_fColor = (m_fColor - (m_fShrinkSpeed * fTimeDelta));
	m_dwColor = D3DCOLOR_ARGB((int)255, (int)m_fColor, (int)m_fColor, (int)m_fColor);
	m_pBufferCom->Set_VertexColor(m_dwColor);
...
void CBill_AlphaLine::Render_Object(void) {
...
void CBill_AlphaLine::Render_Sprite(LPDIRECT3DTEXTURE9 _pTex)
{
	D3DXMATRIX matScale;
	D3DXMATRIX matTrans;
	D3DXMATRIX matWorld;
...
void CBill_AlphaLine::Free(void) {
	Engine::CGameObject::Free();
	Safe_Release(m_pBufferCom);
}


void CBill_AlphaLine::Set_Color(_uint _uNum)
{
	m_pBufferCom->Set_VertexColor(_uNum);
	m_dwColor = _uNum;
	m_fColor = (float)(_uNum >> 24);
}

HRESULT CBill_AlphaLine::Add_Component(void) {
...
CBill_AlphaLine* CBill_AlphaLine::Create(LPDIRECT3DDEVICE9 pGraphicDev) {
	CBill_AlphaLine*		pInstance = new CBill_AlphaLine(pGraphicDev);
